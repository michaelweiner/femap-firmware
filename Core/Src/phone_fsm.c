...
void phone_fsm_init(UART_HandleTypeDef *huart)
{
    huart_at = huart;
}
...
void phone_fsm_process(void)
{
    ...
    bt_hfp_process();
    enum hfpstat_t hfp_stat = bt_hfp_get_stat();
    ...
    case PHONE_IDLE:
        ...
        {
            if (hfp_stat == HFPSTAT_CONNECTED)
            {
                start_dialtone(DIALTONE_DEFAULT);
                phone_state = PHONE_DIALTONE;
            }
            else if (hfp_stat == HFPSTAT_OUTGOING_CALL ||
                     hfp_stat == HFPSTAT_ACTIVE_CALL)
            {
        ...
        else if (hfp_stat == HFPSTAT_INCOMING_CALL)
        {
            start_bell();
        ...
        else
        {
            if (bt_hfp_audio_changed())
            {
                HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13,
                                  bt_hfp_get_audio() == HFPAUDIO_CONNECTED
                                      ? GPIO_PIN_SET
                                      : GPIO_PIN_RESET);
            }
        }
    ...
    case PHONE_INCOMING_CALL:
        if ((hfp_stat != HFPSTAT_INCOMING_CALL) &&
            (bt_hfp_get_audio() != HFPAUDIO_CONNECTED))
        {
            stop_bell();
            phone_state = PHONE_IDLE;
        ...
        else if (pin_gu == GPIO_PIN_SET)
        {
            ...
            stop_bell();
            HAL_UART_Transmit(huart_at, command,
                              strlen((const char *)command), 100);
            HAL_Delay(100);
            HAL_GPIO_WritePin(VOICE_EN_GPIO_Port, VOICE_EN_Pin, GPIO_PIN_SET);
            phone_state = PHONE_ACTIVE_CALL;
        }
    ...
    if ((phone_state == PHONE_IDLE) && bt_hfp_uart_done() &&
        uart_debug_tx_done() && (bt_hfp_get_audio() == HFPAUDIO_DISCONNECTED))
    {
        puts("s");
